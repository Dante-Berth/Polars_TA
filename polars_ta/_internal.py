...
def log_return(close: pl.Expr, periods: int = 1) -> pl.Expr:
...
def rolling_cov(a: pl.Expr, b: pl.Expr, window: int) -> pl.Expr:
...
def rolling_var(a: pl.Expr, window: int) -> pl.Expr:
    """Rolling (population) variance of an expression over ``window`` bars."""
    return rolling_cov(a, a, window)


def rolling_beta(y: pl.Expr, x: pl.Expr, window: int) -> pl.Expr:
